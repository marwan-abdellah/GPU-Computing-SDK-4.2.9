...
#  define checkCudaErrors_NO_SYNC( call) {                                    \
    cudaError err = call;                                                    \
    if( cudaSuccess != err) {                                                \
...
#  define checkCudaErrors( call)     checkCudaErrors_NO_SYNC(call);                                            \
...
#  define CUT_DEVICE_INIT(ARGC, ARGV) {                                      \
    int deviceCount;                                                         \
    checkCudaErrors_NO_SYNC(cudaGetDeviceCount(&deviceCount));                \
    if (deviceCount == 0) {                                                  \
    ...
    checkCudaErrors_NO_SYNC(cudaGetDeviceProperties(&deviceProp, dev));       \
